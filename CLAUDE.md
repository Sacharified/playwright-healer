...
- **Multi-provider** (added in Phase 01.1): three providers selectable via the `provider` input — `anthropic` (default), `gemini`, `ollama`. The `api-key` input is generic and is NOT `required: true` at the runner-level `getInput` call — Zod `superRefine` enforces presence per-provider (Ollama localhost may omit the key). Per-provider default models and endpoints live in `src/shared/config.ts` `DEFAULT_MODELS`. Adapter code lands in Phase 3 under `src/healer/` (Gemini-first).
...
- **Security non-negotiables**: `persist-credentials: false` on all checkout steps; no `pull_request_target` trigger ever; Playwright MCP `--allowed-origins` scoped to `base-url` + localhost; agent `allowedTools` explicitly `["mcp__playwright__*", "Read", "Grep", "Glob"]` (never `Bash`/`Write`/`Edit`) — adapters may rename the `mcp__*` entries per provider syntax but the intent (Playwright MCP + read-only file tools only) is invariant.
