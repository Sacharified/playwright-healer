...
- The action needs an inference-provider API key (`api_key` input — used by `openrouter` for the OpenRouter API key, `github` for a GitHub PAT with `models:read` scope, or omitted entirely for `ollama` localhost). Phase 01.4 narrowed the provider surface to three: `openrouter` (fronts Anthropic, Google, OpenAI, Meta, etc. through one OpenAI-compatible endpoint), `github` (free-tier GitHub Models), and `ollama` (self-hosted localhost). Ollama may omit the key; openrouter and github require it (enforced by Zod `superRefine`). The action uses `GITHUB_TOKEN` for low-scope operations (reading the report, writing stats to the state branch) but requires a PAT or GitHub App token (`healer-token` input) for PR creation and `workflow_dispatch` — PRs opened by `GITHUB_TOKEN` do not trigger downstream CI (GitHub's recursion guard), which would defeat the validation loop

**Key tooling:**
- **LLM agent loop (TypeScript)** — provider-specific adapter selected via the `provider` input. After Phase 01.4: `openrouter` → OpenAI-compatible chat-completions against `https://openrouter.ai/api/v1` with manual MCP tool-use loop (subsumes Anthropic, Gemini, OpenAI, Meta etc. behind one endpoint); `github` → OpenAI-compatible chat-completions against `https://models.github.ai/inference` (free tier); `ollama` → native function-calling via an MCP bridge (Phase 3 stub). All adapters share the same `Adapter.runAgent` contract returning `{ proposal, stats }` with USD cost in `stats.usdSpent`.
...
| Three-provider input surface (OpenRouter, GitHub Models, Ollama) via the `provider` input — Phase 01.1 added 4 providers (anthropic, gemini, github, ollama); Phase 01.4 (2026-05-05) consolidated to 3 by routing all cloud providers through OpenRouter and removing the direct anthropic + gemini adapters | Reusable action shouldn't demand any one vendor subscription; OpenRouter fronts ~30 cloud models behind one OpenAI-compatible endpoint with one API key. GitHub Models stays for the $0 free-tier dev experience (its PAT auth model isn't replicable through OpenRouter). Ollama stays for localhost (cloud-only OpenRouter can't reach it). Net adapter code count drops from 4 → 3 with no loss of model variety; per-call USD comes back uniformly via `usage.cost`. | ✓ Decision-locked 2026-05-05 (Phase 01.4) — supersedes 2026-04-25 lock |
