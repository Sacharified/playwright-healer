...
- [x] **Phase 1.1: Multi-Provider Input Surface** (INSERTED, complete 2026-04-25, **superseded by Phase 01.4 on 2026-05-05**) - Generalized the Anthropic-specific input surface to a four-provider `provider` enum (`anthropic | gemini | github | ollama`); dropped `required: true` on `api-key` with per-provider Zod `superRefine` enforcement; adapter framework landed in Phase 3. Phase 01.4 narrowed the enum to three (`openrouter | github | ollama`) and retired the direct anthropic + gemini SDK adapters in favour of routing all cloud providers through OpenRouter — see Phase 01.4 below for the amendment rationale and the `~/.claude/plans/instead-of-maintaining-integrations-lovely-graham.md` source plan.
- [x] **Phase 1.4: OpenRouter Consolidation** (INSERTED, complete 2026-05-05) - Replace the four-provider input surface from Phase 01.1 with a three-provider surface where one OpenRouter adapter subsumes Anthropic, Gemini, and any future cloud model behind a single OpenAI-compatible endpoint. `github` (free tier) and `ollama` (localhost) keep dedicated paths because OpenRouter cannot serve them. Default `provider` becomes `openrouter` with default model `anthropic/claude-sonnet-4.6`. Hard cutover: `provider: anthropic` and `provider: gemini` removed with no alias. Real-API verification on 2026-05-05 confirmed `usage.cost` and `tool_calls` shape uniformity across Anthropic and Gemini upstreams.
