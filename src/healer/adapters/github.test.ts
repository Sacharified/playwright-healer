...
describe('githubAdapter — submit-tool terminator path', () => {
  function submitFixToolCall(args: Record<string, unknown>, id = 'sub_1') {
    return fetchOk({
      choices: [{
        message: {
          role: 'assistant',
  ...
  function submitNoFixToolCall(evidence: string, id = 'sub_2') {
    return fetchOk({
      choices: [{
        message: {
          role: 'assistant',
